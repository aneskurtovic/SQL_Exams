...
Prilikom kreiranja tabela voditi raèuna o odnosima izmeðu tabela.
...
(možemo ostaviti bilo koji parametar bez unijete vrijednosti). Proceduru pokrenuti za sljedeæe nazive kategorija:
1. Produce
2. Beverages
*/
...
-- 6.b) Napisati naredbu kojom æe se prikazati srednja vrijednost sume zaokružena na dvije decimale.

SELECT ROUND(AVG(suma),2)
FROM view_suma

-- 6.c) Iz pogleda kreiranog pod a) dati pregled zapisa èija je suma veæa od prosjeène sume. Osim kolona iz pogleda, potrebno je prikazati razliku sume i srednje vrijednosti. Razliku zaokružiti na dvije decimale.
...
b) Kreirati proceduru kojom æe se izvršiti punjenje kolone evid_br na sljedeæi naèin:
	...
	- ako je u datumu isporuke unijeta vrijednost, evid_br se dobija spajanjem datum narudžbe i datuma isprouke uz umetanje donje crte izmeðu datuma
...
8. Kreirati proceduru kojom æe se dobiti pregled sljedeæih kolona:
	...
	- kateg_naziv,
	- dobavljac_naziv
Uslov je da se dohvate samo oni zapisi u kojima naziv kategorije sadrži samo 1 rijeè.
Pokrenuti proceduru.
*/
